...
#	for t in lineitem
	do
		...
		impala-shell -i ${cor_node} -f tpch-create-table/${t}.sql -d ${IMPALA_DB_NAME} --var=DB=${IMPALA_DB_NAME} --var=LOCATION=${DIR}/${SCALE} --var=KUDU_DB_NAME=${KUDU_DB_NAME} --var=FILE_FORMAT=${FILE_FORMAT} --query_option=DEFAULT_SPILLABLE_BUFFER_SIZE=1MB 2>&1 | tee -a ${LOG_DIR}/load.log
		date 2>&1 | tee -a ${LOG_DIR}/load.log
...
function runQuery(){
	...
	impala-shell -i ${cor_node} -f sample-queries-tpch/tpch_query${query}.sql -d ${KUDU_DB_NAME} -p --query_option=DEFAULT_SPILLABLE_BUFFER_SIZE=1MB 2>&1 | tee -a ${LOG_DIR}/tpch_query${query}_${stream}.log
	#impala-shell -i ${cor_node} -f sample-queries-tpch/tpch_query${query}.sql -d ${KUDU_DB_NAME} -p 2>&1 | tee -a ${LOG_DIR}/tpch_query${query}_${stream}.log
	end=$(date +%s%3N)
...
function runStream(){
	...
	impala-shell -i ${cor_node} -f stream_query/s${stream_nu}.sql -d ${KUDU_DB_NAME} 2>&1 | tee -a ${LOG_DIR}/stream_${stream_nu}.log
	end=$(date +%s%3N)
